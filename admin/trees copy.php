...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_tree'])) {
        ...
        if (empty($errors)) {
            global $conn;
            $sql = "INSERT INTO trees (reserve_id, species, MTH, THT, DBH, basal_area, volume, status, created_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isddddds", $reserve_id, $species, $MTH, $THT, $DBH, $basal_area, $volume, $status);
    ...
    elseif (isset($_POST['edit_tree'])) {
        ...
        if (empty($errors)) {
            global $conn;
            $sql = "UPDATE trees 
                    SET reserve_id = ?, species = ?, MTH = ?, THT = ?, DBH = ?, basal_area = ?, volume = ?, status = ?
                    WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("isdddddsi", $reserve_id, $species, $MTH, $THT, $DBH, $basal_area, $volume, $status, $id);
...
// Get all trees with reserve information
global $conn;
$sql = "SELECT t.*, r.reserve_name 
        FROM trees t
        JOIN forest_reserves r ON t.reserve_id = r.id
        ORDER BY t.created_at DESC";
$trees = $conn->query($sql);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            ...
                                            <a href="trees.php?delete=<?php echo $tree['id']; ?>" 
    ...
    <?php else: ?>
        <div class="card border-0 shadow">
            <div class="card-body text-center py-5">
                ...
                <p class="text-muted mb-4">Get started by adding your first tree to the inventory.</p>
                <button type="button" class="btn btn-success btn-lg" data-bs-toggle="modal" data-bs-target="#addTreeModal">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="reserve_id" name="reserve_id" required>
                                ...
                                $reserves->data_seek(0);
                                while ($reserve = $reserves->fetch_assoc()): ?>
                                    <option value="<?php echo $reserve['id']; ?>">
                                        <?php echo $reserve['reserve_name']; ?>
                                    </option>
                                <?php endwhile; ?>
                            </select>
