...
// Format currency - Fixed to handle null values
function format_currency($amount) {
    // Check if amount is null or not numeric, and set to 0 if so
...
// Redirect function - Fixed to use BASE_URL properly
...
function calculate_tree_price($volume) {
    // Check if volume is null or not numeric
    ...
    $price = floatval($volume) * $base_price;
...
function is_tree_available($tree_id) {
    // Check if tree_id is valid
    if ($tree_id === null || !is_numeric($tree_id)) {
        return false;
    }
